...
Shader "Custom/C_Water"
{
    Properties
    {
        ...
        _WaveVelocity("Wave Velocity", Range(0.1, 5.0)) = 1.0
                     ...
                     // --- White Foam controls ---
        _WhiteFoamTex ("White Foam Mask (R)", 2D) = "black" {}
        _Water2FoamTex ("Foam Detail (RGB)", 2D) = "white" {}
        _FoamIntensity ("Foam Intensity", Range(0,5)) = 1.0
        _FoamScale     ("Foam UV Scale",  Float)      = 1.0
        _FoamSpeed     ("Foam Scroll Speed", Float)   = 1.0
        _FoamColor     ("Foam Color (RGB)", Color)    = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_WhiteFoamTex);   
            SAMPLER(sampler_WhiteFoamTex);
            TEXTURE2D(_Water2FoamTex);  
            SAMPLER(sampler_Water2FoamTex);

            float  _FoamIntensity;
            float  _FoamScale;
            float  _FoamSpeed;
            float4 _FoamColor;     // rgb used, a not required
            ...
            struct Attributes
            {
                ...
                float2 uv         : TEXCOORD0;
                float2 uv2        : TEXCOORD1; // corrected UV from compute (UV2 on mesh)
                float4 normalOS   : NORMAL;
                float4 tangentOS  : TANGENT;
                float4 color      : COLOR;      // vertex color from compute shader
            };

            struct Varyings
            {
                float2 uv         : TEXCOORD0;
                float2 uvCorr     : TEXCOORD1; // pass to pixel
                float3 positionWS : TEXCOORD2;
                float3 normalWS   : TEXCOORD3;
                float3 tangentWS  : TEXCOORD4;
                float3 bitangentWS: TEXCOORD5;
                float4 color      : COLOR;        // vertex color with alpha mask
            ...
            // Returns RGB foam contribution (already masked & colored)
            float3 CalculateFoam(float2 uvCorr, float2 uvFallback)
            {
                // 1) Artist mask: where & how strong foam appears (R)
                //    Import setting: sRGB Off; Wrap = Clamp (if non-tiling); Filter Bilinear/Trilinear
                float foamMask = SAMPLE_TEXTURE2D(_WhiteFoamTex, sampler_WhiteFoamTex, uvCorr).r;

                // 2) Detail foam pattern (tiling, moving)
                //    Import: sRGB Off; Wrap = Repeat; Filter Trilinear
                float2 baseUV = uvCorr;                 // use corrected domain from compute
                float2 foamUV1 = baseUV * _FoamScale + _Time.y * _FoamSpeed * float2( 0.10,  0.05);
                float2 foamUV2 = baseUV * (_FoamScale * 0.7) + _Time.y * _FoamSpeed * float2(-0.08,  0.12);

                float3 foam1 = SAMPLE_TEXTURE2D(_Water2FoamTex, sampler_Water2FoamTex, foamUV1).rgb;
                float3 foam2 = SAMPLE_TEXTURE2D(_Water2FoamTex, sampler_Water2FoamTex, foamUV2).rgb;

                // Combine two octaves for richer pattern
                float3 foamPattern = saturate(foam1 * foam2 * 2.0);

                // 3) Final foam contribution
                float3 finalFoam = foamPattern * foamMask * _FoamIntensity * _FoamColor.rgb;
                return finalFoam;
            }
            ...
            Varyings vert (Attributes v)
            {
                ...
                o.uv = v.uv;
                o.uvCorr = v.uv2;
                o.color = v.color;   // forward alpha mask from compute
                return o;
            }
            ...
            float4 frag (Varyings i) : SV_Target
            {
                ...
                // Base ocean shading you already have:
                float3 oceanColor = _WeightPBR * pbr_result + _WeightEnvLight * env_result + _WeightNPR * npr_result;;

                // Foam:
                float3 foamColor  = CalculateFoam(i.uvCorr, i.uv);
                foamColor *= i.color.a;                    // mask: 1 inside quad, 0 outside

                // Composite (simple additive looks good for foam)
                float3 col = oceanColor + foamColor;

                // If you prefer masked lerp instead of additive:
                // float foamMask = SAMPLE_TEXTURE2D(_WhiteFoamTex, sampler_WhiteFoamTex, i.uvCorr).r;
                // col = lerp(oceanColor, _FoamColor.rgb, saturate(foamMask * _FoamIntensity)) + foamColor * 0.25;
